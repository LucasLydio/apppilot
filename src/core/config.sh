...
config_setup_paths() {
  ...
  APPPILOT_DEPLOYS_DIR="$APPPILOT_STATE_HOME/deployments"
  APPPILOT_BACKUPS_DIR="$APPPILOT_STATE_HOME/backups"
  APPPILOT_NGINX_AVAILABLE_DIR="${APPPILOT_NGINX_AVAILABLE_DIR:-/etc/nginx/sites-available}"
  APPPILOT_NGINX_ENABLED_DIR="${APPPILOT_NGINX_ENABLED_DIR:-/etc/nginx/sites-enabled}"
  ...
  export APPPILOT_LOCKS_DIR APPPILOT_DEPLOYS_DIR APPPILOT_BACKUPS_DIR APPPILOT_CONFIG_FILE
  export APPPILOT_NGINX_AVAILABLE_DIR APPPILOT_NGINX_ENABLED_DIR
}

config_init() {
  local actions=()
  local path
  for path in "$APPPILOT_CONFIG_HOME" "$APPPILOT_APPS_DIR" "$APPPILOT_SECRETS_DIR" "$APPPILOT_STATE_HOME" "$APPPILOT_LOCKS_DIR" "$APPPILOT_DEPLOYS_DIR" "$APPPILOT_BACKUPS_DIR"; do
  ...
  mkdir -p "$APPPILOT_CONFIG_HOME" "$APPPILOT_APPS_DIR" "$APPPILOT_SECRETS_DIR" "$APPPILOT_STATE_HOME" "$APPPILOT_LOCKS_DIR" "$APPPILOT_DEPLOYS_DIR" "$APPPILOT_BACKUPS_DIR"
