...
config_setup_paths() {
  ...
  export APPPILOT_LOCKS_DIR APPPILOT_DEPLOYS_DIR APPPILOT_CONFIG_FILE
}

config_init() {
  local actions=()
  local path
  for path in "$APPPILOT_CONFIG_HOME" "$APPPILOT_APPS_DIR" "$APPPILOT_SECRETS_DIR" "$APPPILOT_STATE_HOME" "$APPPILOT_LOCKS_DIR" "$APPPILOT_DEPLOYS_DIR"; do
  ...
  mkdir -p "$APPPILOT_CONFIG_HOME" "$APPPILOT_APPS_DIR" "$APPPILOT_SECRETS_DIR" "$APPPILOT_STATE_HOME" "$APPPILOT_LOCKS_DIR" "$APPPILOT_DEPLOYS_DIR"
